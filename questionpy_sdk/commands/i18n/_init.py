...
def _init_explicit(ctx: click.Context, pot: Path, localedir: Path, locale: str, *, force: bool) -> None:
    ...
    if init_cmd.output_file.exists() and not force:
        # TODO: Change after implementing update.
...
def _init_in_source_dir(ctx: click.Context, package: PackageSource, locale: str, *, force: bool) -> None:
...
@click.pass_context
def init(ctx: click.Context, pot_or_package: Path, locales: Collection[str] = (), *, force: bool) -> None:
    if pot_or_package.is_file() and pot_or_package.suffix == ".pot":
        ...
        if not locales:
            ...
            if not force:
                for already_present_locale, _, _ in package.discover_po_files():
            ...
            if not locales:
                ...
                update_cmd = f"{ctx.parent.command_path} update" if ctx.parent else "update"
                msg = (
                    f"The package contains no uninitialized locales. Use '{update_cmd} {package.path}' to update them."
                )
