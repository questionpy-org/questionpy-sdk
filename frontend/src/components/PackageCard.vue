...
<template>
    ...
    <CollapseCard v-else-if="manifest" expanded>
        <BCardBody class="d-flex gap-4 bg-secondary-subtle">
            <div>
                ...
                <BCardText class="d-flex flex-wrap gap-4">
                    <dl class="row mb-0">
                        ...
                        <DElement term="URL" v-if="manifest.url">
                            <BLink :href="manifest.url" target="_blank" rel="noopener">{{ manifest.url }}</BLink>
                        </DElement>
